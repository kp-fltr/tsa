...
export async function GET(request: NextRequest) {
  try {
    console.log('GET /api/analytics/distribution/fa - Starting request')
    ...
    console.log('Fetching distribution:', { dimension, from: fromDate.toISOString(), to: toDate.toISOString() })
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch distribution data', details: error.message },
        { status: 500 }
      )
    }
    ...
    console.log(`FA ${dimension} distribution:`, distributionData)
